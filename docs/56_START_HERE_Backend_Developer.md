...
**NestJS API (Phase 1 product catalogue):** lives under [`backend/`](../backend/). Storefront lives under [`frontend/`](../frontend/). How to add endpoints and modules: [`backend/CONTRIBUTING.md`](../backend/CONTRIBUTING.md). Frontend-facing product/admin routes (what Nest actually serves today): [`docs/71_Nest_Product_Admin_API.md`](./71_Nest_Product_Admin_API.md). Storefront ports still win for shapes; handover OpenAPI is in `docs/openapi.yaml`.
