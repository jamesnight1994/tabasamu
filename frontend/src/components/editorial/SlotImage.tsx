...
export interface SlotImageProps {
  ...
  rounded?: boolean;
  /** Fill a fixed-aspect parent (e.g. arch frame) without imposing slot aspect. */
  fill?: boolean;
  /** Use portraitSrc at all breakpoints (arch / vertical frames). */
  preferPortrait?: boolean;
  /** Tailwind object-position utility, e.g. `object-[center_35%]`. */
...
export function SlotImage({
  ...
  const aspect = fill ? undefined : aspectClasses(slot);
   ...
   */
  const hasArtDirectedMobile =
    !preferPortrait && Boolean(slot.portraitSrc && slot.mobileAspect);

  const imageClass = cn('size-full object-cover', objectPosition);
    ...
    >
      {hasArtDirectedMobile ? (
        ...
        <Image
          ...
          // grid cell is 4:5 on desktop too. Arch frames always prefer portrait.
