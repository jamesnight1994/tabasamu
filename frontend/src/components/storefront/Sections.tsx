...
import { FLAVOUR_CARD_CTA } from './FlavourPromoCard';
...
/** Matches hero, navbar, and flavours horizontal rhythm. */
const SITE_CONTAINER =
  'container mx-auto w-full max-w-[--container-max] px-6 py-16 md:px-12 md:pt-24 md:pb-16 lg:px-16';
...
/* ================================================================== *
 * 6. WHY CHOOSE US — proposition accordion + media
 *
 * Layout reference: screnshots/why-choose-us.png
 * ================================================================== */

function WhyChooseUsMedia({
  caption,
  ctaLabel,
  href,
}: {
  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="relative overflow-hidden rounded-[20px] shadow-[--shadow-overlay] ring-1 ring-inset ring-black/8">
        <SlotImage slot={WHY_CHOOSE_US_SLOT} rounded={false} className="aspect-[150/121]" />
        <div aria-hidden className="why-choose-us-media-overlay pointer-events-none absolute inset-0 z-[1]" />
        <Button
          asChild
          variant="ghost"
          size="md"
          className={cn(
            FLAVOUR_CARD_CTA,
            'absolute bottom-4 right-4 z-[2] w-fit gap-2 shadow-[--shadow-raised]',
            'transition-transform duration-[--duration-fast] hover:scale-[1.02]'
          )}
        >
          <Link href={href}>
            {ctaLabel}
      ...
      <p className="font-body text-[length:--text-body] font-semibold leading-[--leading-snug] text-[--color-ink]">
        {caption}
      </p>
    </div>
  );
...
export function Process() {
  return (
    <section aria-labelledby="why-choose-us-heading" className="bg-surface">
      <div className={SITE_CONTAINER}>
        <div className="grid gap-10 md:grid-cols-2 md:items-start md:gap-16 lg:gap-20">
          <WhyChooseUsMedia
            caption={WHY_CHOOSE_US.mediaCaption}
            ctaLabel={WHY_CHOOSE_US.mediaCtaLabel}
            href={WHY_CHOOSE_US.mediaHref}
          />

          <div className="flex flex-col gap-8 md:pt-12 md:pb-3 md:pr-6">
            <header className="flex flex-col gap-4">
              <h2
                id="why-choose-us-heading"
                className="max-w-[--measure-wide] font-display text-[length:--text-h2] font-normal leading-[--leading-snug] text-[--color-ink]"
              >
                {WHY_CHOOSE_US.titleLead}{' '}
                <span className="text-forest">{WHY_CHOOSE_US.titleAccent}</span>.
              </h2>
              <p className="max-w-[--measure-wide] text-[length:--text-body-lg] leading-[--leading-body] text-[--color-ink-muted]">
                {WHY_CHOOSE_US.intro}
              </p>
            </header>
            ...
            <Accordion
              iconStyle="plus"
              defaultValue={WHY_CHOOSE_US.defaultOpen}
              className="why-choose-us-accordion"
              itemClassName="why-choose-us-accordion-item"
              items={WHY_CHOOSE_US.points.map((point) => ({
                value: point.id,
                trigger: point.title,
                content: <p>{point.body}</p>,
              }))}
            />
          </div>
        </div>
